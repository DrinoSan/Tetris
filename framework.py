...
class BaseGame:
    ...
    def draw_game_board(self):
        ...
        pygame.draw.rect(
            ...
            (
                self.margin,
                self.top_margin,
                self.box_size * self.board_width,
    ...
    def convert_coords(self, board_x, board_y):
        return (self.margin + (board_x * self.box_size)), (
            self.top_margin + (board_y * self.box_size)
        )
    ...
    def draw_cell(self, x, y, color, next_x=None, next_y=None):
        ...
        pygame.draw.rect(
            self.display,
            self.colors[color],
            (next_x + 1, next_y + 1, self.box_size, self.box_size),
        )
        pygame.draw.rect(
            self.display,
            self.accent_colors[color],
    ...
    def draw_level(self):
        text = self.font.render(
            "Level: {}".format(self.level), True, self.colors["white"]
        )
        rect = text.get_rect()
        rect.topleft = (20, 140)
        self.display.blit(text, rect)
