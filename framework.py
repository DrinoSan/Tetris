...
class BaseGame():
    ...
    def test_quit_game(self):
        for evnt in pygame.event.get(QUIT):
            pygame.quit()
            sys.exit()
    ...
    def draw_game_board(self):
        pygame.draw.rect(self.display, self.border_color, (
            self.margin - 3, self.top_margin -
            7, (self.board_width * self.box_size) + 8,
    ...
    def draw_cell(self, x, y, color, next_x=None, next_y=None):
        ...
        pygame.draw.rect(
            self.display, self.colors[color], (next_x + 1, next_y + 1, self.box_size, self.box_size))
        ...
        for x in range(block.width):
            for y in range(block.height):
                if block.shape[y][x] != self.blank_color:
                    self.draw_cell(None, None, block.color, p_x +
                                   (x * self.box_size), p_y + (y * self.box_size))
    ...
    # Do not modify
    def draw_level(self):
        text = self.font.render('Level: {}'.format(
            self.level), True, self.colors['white'])
        rect = text.get_rect()
        rect.topleft = (20, 140)
        self.display.blit(text, rect)
